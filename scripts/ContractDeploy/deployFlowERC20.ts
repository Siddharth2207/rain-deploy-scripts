...
export const deployFlowERC20 = async (fromNetwork:string, toNetwork:string) => {    

    const txHash  = contractConfig.contracts[fromNetwork].flowERC20.transaction
    ...
    console.log(`flowERC20 deployed to ${toNetwork} at : ${transactionReceipt.contractAddress}`)   

    let updateContractConfig = contractConfig["contracts"] 

    updateContractConfig[toNetwork] ? (
      updateContractConfig[toNetwork]["flowERC20"] = {
